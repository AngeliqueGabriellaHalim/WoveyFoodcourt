    ...
    private String homepage(Model model) {
        ...
        if (terlaku.size() >= 3) {
            model.addAttribute("terlaku1",
                    terlaku.get(0).getIdproduk() + " - " + terlaku.get(0).getKuantitas() + " Terjual");
            model.addAttribute("terlaku2",
                    terlaku.get(1).getIdproduk() + " - " + terlaku.get(1).getKuantitas() + " Terjual");
            model.addAttribute("terlaku3",
                    terlaku.get(2).getIdproduk() + " - " + terlaku.get(2).getKuantitas() + " Terjual");
        }

        if (penjualan.size() >= 3) {
            model.addAttribute("topUMK1", "UMK " +
                    penjualan.get(0).getNamaUMK() + " - " + penjualan.get(0).getJumlahTransaksi() + " Terjual");
            model.addAttribute("topUMK2", "UMK " +
                    penjualan.get(1).getNamaUMK() + " - " + penjualan.get(1).getJumlahTransaksi() + " Terjual");
            model.addAttribute("topUMK3", "UMK " +
                    penjualan.get(2).getNamaUMK() + " - " + penjualan.get(2).getJumlahTransaksi() + " Terjual");
